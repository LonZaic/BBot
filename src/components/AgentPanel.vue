<template>
  <div v-if="visible" class="agent-panel" :class="{ collapsed }">
    <div class="agent-header" @click="collapsed = !collapsed">
      ...
      <span class="title">{{ running ? (action || 'Working...') : (error ? 'Error' : (done ? 'Done ✓' : 'Agent')) }}</span>
    ...
    <div v-if="!collapsed" class="body" ref="bodyRef">
      ...
      <div v-if="running" class="scan"></div>
    </div>
  </div>
</template>
...
watch(() => props.events, (evts) => {
  ...
  for (const e of evts) {
    ...
    if (e.type === 'done' || e.type === 'final') { running.value = false; done.value = true }
    if (e.type === 'error') { running.value = false; error.value = true }
    if (e.type === 'aborted') { running.value = false; done.value = true }
  }
...
.err { color:var(--red); }
.ok { color:var(--green); }
.scan { height:1px; background:linear-gradient(90deg,transparent,var(--primary),transparent); animation: scan .5s ease-in-out infinite; margin-top:4px; }
